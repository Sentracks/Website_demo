    ...
    <div className="card">
      <div className="header-row">
        <div>
          ...
          <div className="muted">
            Each record includes route, temperature control, and risk level.
          </div>
        </div>
      </div>
        ...
        <table>
          ...
          <tbody>
            {db.shipments.map((shipment) => {
              ...
              const logs = db.temperatureLogs.filter(
                (log) => log.shipmentId === shipment.id
              );

              const analysis =
                db.shipmentAnalyses.find(
                  (item) => item.shipmentId === shipment.id
                ) ?? analyzeShipment(shipment, logs);
